...
Monitor.prototype.setStats = function(stats) { // arguments contains stats names
    ...
    self.stats = {};
    self.interval = {};
    stats = Array.isArray(stats) ? stats : [].slice.call(arguments);
    var len = stats.length;

    // don't use [].forEach here, it is ~20x slower than for loop, can cause problems with smaller timeout updates
    // TODO: Should Monitor incorporate some sort of lock to prevent calling setStats during an update operation?
    for (var i = 0; i < len; i++) {
        var stat = stats[i];
...
/**
 * Called by the instrumented code when it begins executing.
 *
 * Returns a monitoring context.
 *
 * Call `context.end()` when the instrumented code completes.
 */
Monitor.prototype.start = function(args) {
    var self = this,
        ...
        doStart = function(m, context) {
            if (m.start) {
                m.start(context, args);
            }
            if (m.end) {
                endFuns.push(function(result) {
                    return m.end(context, result);
                });
            }
        },
        monitoringContext = {
            end: function(result) {
                endFuns.forEach(function(f) {
                    f(result);
                });
            }
        };

    self.collectors.forEach(function(m) {
        doStart(m, {});
    });
 ...
 * for monitoring concurrently executing instances of `Loop`
 *
...
/**
 * Set the file name or `LogFile` object that statistics are logged to; null for default
 */
