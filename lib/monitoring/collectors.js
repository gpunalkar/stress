...
/**
 * Track the runtime of an operation, storing stats in a `Histogram`
 */
StatsCollectors['runtime'] = StatsCollectors['latency'] = function RuntimeCollector(params) {
    ...
    self.end = function(context) {
        self.stats.put(new Date() - context.start);
    };
};

/**
 * Track HTTP response codes, storing stats in a `ResultsCounter` object.
 *
...
StatsCollectors['result-codes'] = function ResultCodesCollector() {
    ...
    self.end = function(context, http) {
        self.stats.put(http.res.statusCode);
    };
};

/**
 * Track requests per second, storing stats in a `Rate` object.
 *
 * The client must call
 *
 * `.end({res: http.ClientResponse})`
...
/**
 * Track a status code that is returned in an HTTP header, storing stats in a `ResultsCounter` object.
 *
 * The client must call
 *
 * `.end({res: http.ClientResponse})`
...
/**
 * Track the concurrent executions (ie. stuff between calls to `.start()` and `.end()`), storing in a `Peak`.
 */
...
/**
 * Track the size of HTTP request bodies sent by adding up the content-length headers.
 * This function doesn't really work as you'd hope right now, since it doesn't work for chunked encoding messages and
...
StatsCollectors['request-bytes'] = function RequestBytesCollector() {
    ...
    self.end = function(context, http) {
        if (http && http.req) {
            if (http.req._header) {
                self.stats.put(http.req._header.length);
            }
            if (http.req.body) {
                self.stats.put(http.req.body.length);
            }
        }
    };
};
...
/**
 * Track unique URLs requested, storing stats in a `Uniques` object.
 *
 * The client must call
 *
 ...
 * `params.successCodes`.
 *
 * Specify the log file name in `params.log`, or leave undefined to disable logging.
 */
