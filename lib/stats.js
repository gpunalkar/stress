...
Histogram.prototype = {
    ...
    get: function(item) {
        if (item < this.items.length) {
            ...
            for (var i in this.extra) {
                if (!this.extra.hasOwnProperty(i)) {
                    continue;
                }
    ...
    percentile: function(percentile) {
        ...
        if (this.extra.length > target) {
            ...
            if (!this.sorted) {
                this.extra = this.extra.sort(function(a, b) {
                    return a - b;
                });
                this.sorted = true;
            }
...
ResultsCounter.prototype = {
    ...
    merge: function(other) {
        for (var i in other.items) {
            if (other.items.hasOwnProperty(i)) {
                if (this.items[i] !== undefined) {
                    this.items[i] += other.items[i];
                } else {
                    this.items[i] = other.items[i];
...
Rate.prototype = {
    ...
    get: function() {
        // calls/(now-start) is the rate.
        // To calculate rate per second with 0.1 precision, multiply rate by 10000, round it, then divide by 10
        return Math.round((this.length / (+new Date() - this.start))*10000)/10;
    },
            ...
            if (arguments.length < 2) {
                for (var i in this) {
                    if (this.hasOwnProperty(i)) {
                        this[i].put(statNameOrVal);
                    }
                }
            } else {
                if (this[statNameOrVal]) {
                    this[statNameOrVal].put(val);
                }
            }
        }
    });
...
var mergeStatsGroups = stats.mergeStatsGroups = function(sourceGroup, targetGroup) {
    for (var statName in sourceGroup) {
        if (sourceGroup.hasOwnProperty(statName)) {
...
var randomString = stats.randomString = function(length) {
    ...
    for (var i = 0; i < length; i++) {
        // Math.random() returns ~0.001 through ~0.999.  Multiplying by 95 then flooring gives us
        // a value between 0 and 94. Add 32 to that and we have the ASCII range.
        chars.push(String.fromCharCode((Math.floor(Math.random() * 95) + 32)));
    }
