 ...
 *   @param masterHttpServer the http.js#HttpServer instance that will receive mesages from slaves. Defaults to global `HTTP_SERVER`.
 *   @param slaveUpdateInterval Number of milliseconds between each 'update' event (which contains the latest statistics) from this cluster and also the the interval at which each slaves should ping us to let us know it is still alive.
 *
...
var LoadTestCluster = exports.LoadTestCluster = function LoadTestCluster(masterHost, slaveHosts, masterHttpServer, slaveUpdateInterval) {
    ...
    self.slaveUpdateInterval = slaveUpdateInterval || config.settings.monitorInterval;
};
LoadTestCluster.prototype.__proto__ = EventEmitter.prototype;
...
LoadTestCluster.prototype.run = function(specs) {
    ...
    if (self.cluster && self.cluster.started()) {
    ...
    self.cluster.on('init', function() {
        self.cluster.on('start', function() {
            ...
            self.setLoggingEnabled(config.settings.logsEnabled);
        });
        self.cluster.start();
    });
...
/**
 * Get an actual cluster.js#Cluster definition that will create an local master endpoint and be sent to the slaves
 */
