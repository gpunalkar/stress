...
var SlaveNode = exports.SlaveNode = function SlaveNode(server, spec) {
    ...
    this.slaveEndpoint_.on('start', function() {
        this.emit.bind(this, 'start');
    });
    this.slaveEndpoint_.on('end', this.end.bind(this));

    this.slaveEndpoint_.start();
    this.slaveEndpoint_.context.id = this.id;
    this.slaveEndpoint_.context.__defineGetter__('state', function() {
        return slaveState;
    });
    ...
    this.updateInterval = (spec.pingInterval >= 0) ? spec.pingInterval : config.settings.slaveUpdateInterval;
};
SlaveNode.prototype.__proto__ = EventEmitter.prototype;
 ...
 */
function installRemoteHandler(server) {
    var slaveNodes = [];
    server.addRoute('^/remote/?$', function(path, req, res) {
        if (req.method === 'POST') {
            ...
            res.end(JSON.stringify(slaveNodes.map(function(s) {
                return s.url;
            })));
        } else {
            res.writeHead(405);
...
exports.installRemoteHandler = installRemoteHandler;