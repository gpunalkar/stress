...
*      }
 *
 *  Calling `cluster.start()` will register a master handler on the provided `HttpServer`. It will
 *  connect to every slave, asking each slave to
 *
 *     1) execute the setup() function
 *     2) report its current state to this host every pingInterval milliseconds.
 *
 *  Calling `cluster.slave_remote_function_1()`, will execute slave_remote_function_1 on every slave.
 *
...
var Cluster = exports.Cluster = function Cluster(spec) {
    ...
    self.pingInterval = spec.pingInterval || config.settings.slaveUpdateInterval;
    ...
    util.forEach(spec.slaves, function(method, val) {
        if (typeof val === 'function') {
            ...
            self[method] = function() {
                self.slaves[method].apply(self.slaves, arguments);
            };
        }
    });
...
Cluster.prototype.__proto__ = EventEmitter.prototype;

Cluster.prototype.started = function() {
    return this.state === 'started';
};

/**
 * Start cluster; install a route on the local HTTP server and send the slave definition to all the slave instances.
 */
...
/**
 * Stop the cluster; remove the route from the local HTTP server and uninstall and disconnect from all the slave instances
 */
...
/**
 * Receive a periodic state update message from a slave.
 * When all slaves enter the `running` or `done` states, emit an event.
 */
