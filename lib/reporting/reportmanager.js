// Reports added to the global REPORT_MANAGER are served by the global HTTP_SERVER instance
// (defaults to http://localhost:8000/) and written to disk at regular intervals.
...
config.once('apply', function(){
    "use strict";
    REPORT_MANAGER.refreshIntervalMs = REPORT_MANAGER.refreshIntervalMs || config.settings.refreshInterval;
    REPORT_MANAGER.setReportBaseDir(config.settings.baseDirectory);
    REPORT_MANAGER.setLoggingEnabled(config.settings.logsEnabled);
});
