...
/**
 * defines all of the parameters that used with Loop.create(), MultiLoop()
 *
 * @type {{fun: undefined, argGenerator: undefined, args: undefined, rps: Number, duration: Number, numberOfTimes: Number, concurrency: number, concurrencyProfile: undefined, rpsProfile: undefined}}
 */
var LOOP_OPTIONS = exports.LOOP_OPTIONS = {

    /**
     *  A function to execute which accepts the parameters (finished, args).
    ...
    /**
     * A function which is called once when the loop is started. The return
     * value is passed to fun as the "args" parameter. This is useful when
     * concurrency > 1, and each "thread" should have its own args.
     */
    argGenerator: undefined,

    /**
     * If argGenerator is NOT specified, then this is passed to the `fun` as `args`
     */
    args: undefined,

    /**
     * Target number of time per second to call fun()
     */
    rps: Infinity,

    /**
     * Maximum duration of this loop in seconds
     */
    duration: Infinity,

    /**
     * Maximum number of times to call fun()
     */
    numberOfTimes: Infinity,

    /**
     * (MultiLoop only) Number of concurrent calls of fun()
     */
    concurrency: 1,

    /**
     * (MultiLoop only) array indicating concurrency over time:
 ...
 *
 * @param funOrSpec    Either a loop specification object or a loop function. LOOP_OPTIONS lists all the supported fields in a loop specification.
 * @param args         passed as-is as the second argument to fun
 * @param conditions   a list of functions that are called at the beginning of every loop. If any function returns false, the loop terminates. Loop#timeLimit and Loop#maxExecutions are conditions that can be used here.
...
var Loop = exports.Loop = function Loop(funOrSpec, args, conditions, rps) {
    ...
    if (this.args && this.args instanceof EventEmitter) {
        this.args.on('reconnected', function(arg) {
            this.args = arg;
            this.stop();
            this.start();
        });
...
Loop.prototype.checkConditions_ = function() {
    return this.running && this.conditions.every(function(c) {
        return c();
    });
};
...
/**
 * A wrapper for any existing function so it can be used by Loop.
 * Examples:
 *
