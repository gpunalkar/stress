...
/**
 * TEST_OPTIONS defines all of the parameters that can be set in a test specifiction passed to `run()`.
 *
 * By default (calling require('nodeload').run({});), will GET localhost:8080/ as fast as possible with 10 users for 2 minutes.
 *
 * @type {{name: string, host: string, port: number, connectionGenerator: undefined, requestGenerator: undefined, requestLoop: undefined, method: string, path: string, requestData: undefined, numUsers: number, loadProfile: undefined, targetRps: Number, userProfile: undefined, numRequests: Number, timeLimit: number, delay: number, stats: Array}}
 */
var TEST_OPTIONS = {
    ...
    /**
     * Host to connect via HTTP
     */
    host: 'localhost',

    /**
     * Port to connect to `Host` over HTTP
     */
    port: 8080,

    /**
     * `connectionGenerator()` is called once for each user.
     ...
     *
     * Specify either host/port or connectionGenerator, but not both
     */
    connectionGenerator: undefined,

    /**
     * requestGenerator: a function
     * `function(http.Client)` ->  http.ClientRequest
     */
    requestGenerator: undefined,

    /**
     * requestLoop: is a function
     * `function(finished, http.Client)`
     */
    requestLoop: undefined,

    /**
     * If set, must call:
     *
     *      finished({   req: http.ClientRequest, res: http.ClientResponse});
     *
     * after each transaction to finishes to schedule the next iteration of requestLoop.
     *
    ...
    /**
     * numUsers: number of virtual users concurrently executing therequest loop
     */
    numUsers: 10,

    /**
     * loadProfile: array with requests/sec over time:
    ...
    /**
     * targetRps: times per second to execute request loop
     */
    targetRps: Infinity,

    /**
     * userProfile: array with number of users over time:
 ...
 * See `TEST_OPTIONS` for a list of the configuration values in each specification.
 *
 ...
 */
var createClient = exports.createClient = function(port, host) {
