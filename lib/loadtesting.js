...
var run = function(specs) {
    ...
    if (!(specs.every(function(s) {
        return s instanceof Spec;
    }))) {
    ...
    var tests = specs.map(function(spec) {
        ...
        var generateRequest = function(client) {
            ...
            loop = new MultiLoop({
                ...
                concurrencyProfile: spec.userProfile || [
                    [0, spec.numUsers]
                ],
                rpsProfile        : spec.loadProfile || [
                    [0, spec.targetRps]
                ],
        ...
        // TODO: This isn't necessary because config is a cached module...
        monitor.setLoggingEnabled(config.settings.logsEnabled);
...
var LoadTest = function LoadTest(tests) {
    ...
    var self = this;
    self.tests = tests;
    self.updateInterval = config.settings.monitorInterval;
    self.interval = {};
    self.stats = {};
...
LoadTest.prototype.start = function(keepAlive) {
    ...
    if (!HTTP_SERVER.running && config.settings.enableServer) {
